#!/usr/bin/env python3
"""
Plot smoothed average of all SoilMidVoltage files from sample_outputs
"""
import os
import glob
import csv
...
print("Loading SoilMidVoltage data from sample_outputs...", flush=True)

# Find only SoilMidVoltage CSV files
csv_files = sorted(glob.glob('sample_outputs/*SoilMidVoltage*.csv'))
print(f"Found {len(csv_files)} SoilMidVoltage files", flush=True)

if not csv_files:
    print("No SoilMidVoltage files found!")
    exit(1)
...
try:
    ...
    # Update layout
    fig.update_layout(
        title=f'Soil Mid Voltage - Smoothed Average Energy vs Intensity ({len(csv_files)} files)',
    ...
    # Save as HTML
    output_file = 'soil_mid_voltage_average.html'
