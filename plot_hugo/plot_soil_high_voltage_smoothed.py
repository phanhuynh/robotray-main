#!/usr/bin/env python3
"""
Plot smoothed average of all SoilHighVoltage files from sample_outputs
"""
import os
import glob
import csv
...
print("Loading SoilHighVoltage data from sample_outputs...", flush=True)

# Find only SoilHighVoltage CSV files
csv_files = sorted(glob.glob('sample_outputs/*SoilHighVoltage*.csv'))
print(f"Found {len(csv_files)} SoilHighVoltage files", flush=True)

if not csv_files:
    print("No SoilHighVoltage files found!")
    exit(1)
...
try:
    ...
    # Update layout
    fig.update_layout(
        title=f'Soil High Voltage - Smoothed Average Energy vs Intensity ({len(csv_files)} files)',
    ...
    # Save as HTML
    output_file = 'soil_high_voltage_average.html'
