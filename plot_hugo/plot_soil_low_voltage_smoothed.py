#!/usr/bin/env python3
"""
Plot smoothed average of all SoilLowVoltage files from sample_outputs
"""
import os
import glob
import csv
...
print("Loading SoilLowVoltage data from sample_outputs...", flush=True)

# Find only SoilLowVoltage CSV files
csv_files = sorted(glob.glob('sample_outputs/*SoilLowVoltage*.csv'))
print(f"Found {len(csv_files)} SoilLowVoltage files", flush=True)

if not csv_files:
    print("No SoilLowVoltage files found!")
    exit(1)
...
try:
    ...
    # Update layout
    fig.update_layout(
        title=f'Soil Low Voltage - Smoothed Average Energy vs Intensity ({len(csv_files)} files)',
    ...
    # Save as HTML
    output_file = 'soil_low_voltage_average.html'
