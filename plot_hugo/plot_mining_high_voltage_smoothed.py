#!/usr/bin/env python3
"""
Plot smoothed average of all MiningHighVoltage files from sample_outputs
"""
import os
import glob
import csv
...
print("Loading MiningHighVoltage data from sample_outputs...", flush=True)

# Find only MiningHighVoltage CSV files
csv_files = sorted(glob.glob('sample_outputs/*MiningHighVoltage*.csv'))
print(f"Found {len(csv_files)} MiningHighVoltage files", flush=True)
